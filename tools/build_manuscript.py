...
body("시제품의 외형과 6층 적층 구조는 Fig. 2, 주요 부품 구성은 Table 2와 같다. 제어부는 비전·미션 처리를 담당하는 Raspberry Pi 4와 "
     ...
     "12 V 배터리를 분리한 듀얼 배터리 전원 구조(Fig. 3)를 채택해 모터 노이즈의 제어부 유입을 "
     "억제하였다.")
figure_ph("Fig. 2 Appearance of the prototype robot and its six-layer "
          "mechanical stack")
...
figure_ph("Fig. 3 Dual-battery power architecture separating the 7.4 V logic "
...
body("전역 경로 탐색에는 격자 지도 상의 A* 알고리즘{1}을 사용한다. 평가 함수는 f(n) = g(n) + h(n) "
     ...
     "계획기로의 이식을 용이하게 하기 위한 의도적 설계이다. 전역 경로와 장애물 팽창 비용의 적용 "
     "예를 Fig. 4에 나타냈다.")
figure_ph("Fig. 4 A* global path with obstacle inflation on the prototype map")
...
body("각 음식물 쓰레기통에는 고유 QR 코드{3}를 부착하여 빈 식별과 접근 정렬에 활용한다. 로봇은 QR "
     ...
     "장애물을 감지한다. QR 코드 기반 빈 식별과 접근 정렬 과정을 Fig. 5에 나타냈다.")
figure_ph("Fig. 5 QR-code-based bin identification and approach alignment")
heading("3.4 미션 상태머신 및 안전 시스템", "논문의 각 절")
body("수거 미션은 Fig. 6과 같이 IDLE → NAV_TO_BIN → APPROACH → PICKUP → NAV_TO_DEPOT → DROP "
...
figure_ph("Fig. 6 Finite-state machine of the collection mission")

# --- 4. 실험 및 분석 ---
heading("4. 실험 및 분석", "논문의 각 장")
heading("4.1 실험 환경", "논문의 각 절")
body(f"실험은 시제품 테스트 환경을 모사한 {R['meta']['grid']} 격자(장애물 셀 "
     f"{R['meta']['n_obstacles']}개, 건물 4동·놀이터·주차장 포함)에서 수행하였다(Fig. 7). 빈 4개와 로봇 "
...
e2lo, e2hi = E2[0], E2[-1]
body(f"팽창 반경 r_infl 을 0 에서 4 까지 변화시키며 단일 로봇 미션 경로를 분석하였다(Table 4, Fig. 8(a)). "
...
body(f"실제 4-bin 레이아웃에서 최근접 이웃 휴리스틱은 완전 탐색 최적해와 동일한 경로를 산출하여 "
     ...
     f"N=8 의 {g8['mean_gap_pct']}% 로 증가하였다(Table 5, Fig. 8(b)). 이는 소규모 빈 배치에서는 최근접 이웃 "
...
body(f"풀스케일 단지로의 확장 가능성을 확인하기 위해 빈 격자 크기를 {e4lo['grid']} 에서 "
     f"{e4hi['grid']} 까지 늘리며 대각선 경로 질의의 계산 시간을 측정하였다(Table 6, Fig. 8(c)). 격자 셀 수가 "
